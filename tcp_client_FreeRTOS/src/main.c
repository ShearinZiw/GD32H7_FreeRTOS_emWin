...
static TaskHandle_t Test2_Task_Handle = NULL;/* LED任务句柄 */
static TaskHandle_t Test3_Task_Handle = NULL;/* KEY任务句柄 */
static TaskHandle_t Test4_Task_Handle = NULL;/* DHCP任务句柄 */
...
static void AppTaskCreate(void)
{
  ...
  printf("如需修改IP地址与端口号，则修改对应的宏定义:IP_ADDR0,IP_ADDR1,IP_ADDR2,IP_ADDR3,IP_S_PORT等\n\n");  
  ...
  if(pdPASS == xReturn)
    printf("Create Test3_Task sucess...\n\n");

#ifdef USE_DHCP
  /* 创建Test4_Task任务 */
  xReturn = xTaskCreate((TaskFunction_t )dhcp_task,  /* 任务入口函数 */
                        (const char*    )"Test4_Task",/* 任务名字 */
                        (uint16_t       )512,  /* 任务栈大小 */
                        (void*          )NULL,/* 任务入口函数参数 */
                        (UBaseType_t    )1, /* 任务的优先级 */
                        (TaskHandle_t*  )&Test4_Task_Handle);/* 任务控制块指针 */ 
  if(pdPASS == xReturn)
    printf("Create Test4_Task sucess...\n\n");
#endif /* USE_DHCP */
...
{	 
    vTaskDelay(3000);/* 延时3000个tick */
    printf("    KEY1: 新建一个TCP连接\n");
    printf("    KEY2: 断开最后一个TCP连接\n");
    while(1) 
    {
