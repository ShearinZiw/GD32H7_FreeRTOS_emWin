...
static int I2C_Transfer( struct i2c_msg *msgs,int num)
{
	...
	for (im = 0; ret == 0 && im != num; im++)
	{
		...
		{
			ret = GPT_I2C_ReadBytes(msgs[im].addr, msgs[im].buf, msgs[im].len);		//IIC读取数据
		} 
        else
		{
			ret = GPT_I2C_WriteBytes(msgs[im].addr,  msgs[im].buf, msgs[im].len);	//IIC写入数据
		}
	}
...
/**
  * @brief   读取芯片版本。
  * @param 无
  * @retval 2: succeed, otherwise: failed
  */
